...
GLuint loadShaders(const std::string &vertex_shader_code, const std::string &fragment_shader_code)
{
 // Create the shaders
    ...
    GL_CHECK(glShaderSource(vertex_shader_id, 1, &vertex_source_pointer, NULL));
    GL_CHECK(glCompileShader(vertex_shader_id));
    checkShader(vertex_shader_id);

    // Compile Fragment Shader
    char const * fragment_source_pointer = fragment_shader_code.c_str();
    GL_CHECK(glShaderSource(fragment_shader_id, 1, &fragment_source_pointer, NULL));
    ...
    GL_CHECK(glGetProgramiv(program_id, GL_LINK_STATUS, &result));
    GL_CHECK(glGetProgramiv(program_id, GL_INFO_LOG_LENGTH, &info_log_length));

    if (info_log_length > 0) {
        std::vector<char> program_error_message(info_log_length+1);
        GL_CHECK(glGetProgramInfoLog(program_id, info_log_length, NULL, program_error_message.data()));
